...
- **Status:** Accepted (2026-09-03 — owner go-ahead after full presentation of research + proposal). **M9a implemented in 1.3.0** (`jarvis mcp serve`); **M9c implemented in 1.4.0** (`jarvis doctor`, context-store write-scan + hash chain, suggestion canaries); **M9d implemented in 1.5.0** (`jarvis charter`, circuit-broken standing orders); M9b/M9e await per-phase go. Research basis: [RESEARCH-agent-landscape-2026.md](../RESEARCH-agent-landscape-2026.md) (OpenClaw, Hermes, Open Interpreter, AutoGPT, OS-Copilot/UFO²/OSWorld field, MCP, skill-security ecosystem; all claims sourced there).
