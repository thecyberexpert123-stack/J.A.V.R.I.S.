...
| **M5** GUI control | AT-SPI backend, ≥3 compositor backends, ydotool wizard, vision fallback | Task set of 15 GUI tasks (GNOME/KDE/X11) passes ≥98% *within the catalog*; wizard works on fresh Fedora & Ubuntu Wayland sessions |
| **M6** Production hardening & packaging | pipx package, distro packages (deb/rpm/AUR at least), docs in-repo, telemetry strictly opt-in (or none — owner decides), v1.0 release on branch + PR to `main` (opened, **never merged by agent**) | Full CI matrix green; install tested on clean containers of all Tier-1 distros; CHANGELOG complete; zero known P0/P1 issues open |
